...
mo_f1, mo_f2 = 'Helvetica','Helvetica'
    ...
    # オン一覧(基本は1にしておく)
        ...
        for a in area:
            if a=='main':
                ...
                button_re.place(x=55, y=210)
                button_of_re.place(x=60, y=260)
                button_ed.place(x=195, y=210)
                button_che.place(x=335, y=210)
            elif a=='to_ho':
                h1.place(x=10, y=10)
                button_home.place(x=w_x-110, y=25)
            elif a=='isbn':
                ...
                p2.place(x=85, y=y_re+y_bl)
                p3.place(x=190, y=y_re+y_bl*2)
                p4.place(x=85, y=y_re+y_bl*3)
                p6.place(x=70, y=y_re+y_bl*4)
                sel1.place(x=230, y=y_re+y_bl*3)
                memo1.place(x=100, y=y_re+y_bl*5+2)
...
def bookse(target):
    ...
    if len(e)==13 and 9780000000000 <= int(e) < 9800000000000:
        ...
        button3.place(x=w_x-110, y=w_y-y_bl)
...
def add_combobox_subgen(target):
    ...
    gen_se = sel1.get()
    if gen_se == '新規作成':
        p5.place(x=350, y=y_re+y_bl*3-2)
        p5.delete(0,tk.END)
    else:
        p5.place_forget()
...
def bookre(target):
    ...
    if b_d != '' and isbn != '' and b_tit != '' and b_aut != '' and b_p != '' and b_gen != '' and b_me != '':
        ...
        with open('mybookrecord.csv','a', encoding='utf-8') as f:
...
def mode_edit(target):
    ...
    button_onof(['to_ho'],1)
    h1["text"] = app_mode[1]
    h10.place(x=230, y=25)
    sel_re.place(x=300, y=25)
    # データを読み込む
    if len(all_data)!=1:
        num = len(all_data)-1
...
def search_re(target):
    ...
    button4.place(x=w_x-225, y=w_y-y_bl)
    button5.place(x=w_x-110, y=w_y-y_bl)
...
def bookrere(target):
    ...
    if b_d != '' and isbn != '' and b_tit != '' and b_aut != '' and b_p != '' and b_gen != '' and b_me != '':
            ...
            with open('mybookrecord.csv','w', encoding='utf-8') as f:
                for a in all_data:
...
# 基本画面
root = tk.Tk()
root.title("読書記録アプリ mac対応版")
root.minsize(w_x, w_y)

# ジャンルデータ読み込み、csvがなければ作成
# 基本的にcsvファイル自体を開かないこと
# どうしても確認したい場合は、UTF-8で開く
try:
    with open('mybookrecord.csv','r', encoding='utf-8') as f:
...
# ホームレイアウト
title = tk.Label(text='読書記録アプリ',font=(mo_f1,36))

button_re = Button(text=app_mode[0],font=(mo_f1,16))
button_re.bind('<ButtonRelease-1>',mode_re)

button_of_re = Button(text=app_mode[3],font=(mo_f1,14))
button_of_re.bind('<ButtonRelease-1>',book_of_re)

button_ed = Button(text=app_mode[1],font=(mo_f1,16))
button_ed.bind('<ButtonRelease-1>',mode_edit)

button_che = Button(text=app_mode[2],font=(mo_f1,16))
button_che.bind('<ButtonRelease-1>',all_check)

button_onof(['main'],1)


# 共通項目色々
h1 = tk.Label(text='',font=(mo_f1,28))

button_home = Button(text='ホームへ',font=(mo_f1,14),padx=0,pady=0)
button_home.bind('<ButtonRelease-1>',to_home)

h_do = tk.Label(text='待機中',font=(mo_f1,14),fg='#f00')
h_do.place_forget()

# 記録モード用
h2 = tk.Label(text='ISBN_13：',font=(mo_f2,14))

p1 = tk.Entry(width=17,font=(mo_f2,14))

button2 = Button(text='検索する',font=(mo_f1,14),padx=0,pady=0,fg=c_t)
button2.bind('<ButtonRelease-1>',bookse)

h3 =tk.Label(text='タイトル：',font=(mo_f2,14))

p2 = tk.Entry(width=45,font=(mo_f2,14))

h4 = tk.Label(text='著者（複数名は / 区切り）:',font=(mo_f2,14))

p3 = tk.Entry(width=32,font=(mo_f2,14))

h5 = tk.Label(text='ページ数：',font=(mo_f2,14))

p4 = tk.Entry(width=5,font=(mo_f2,14),justify=tk.CENTER)

h6 = tk.Label(text='ジャンル：',font=(mo_f2,14))

sel1 = ttk.Combobox(root, values = gen, width = 10, state='readonly')
sel1.set(gen[0])
sel1.bind('<<ComboboxSelected>>', add_combobox_subgen)

p5 = tk.Entry(width=15,font=(mo_f2,14))
p5.place_forget()

h7 = tk.Label(text='読了日：',font=(mo_f2,14))

p6 = tk.Entry(width=12,font=(mo_f2,14),justify=tk.CENTER)

h8 = tk.Label(text='感想・メモ：',font=(mo_f2,14))

memo1 = tk.Text(width=44, height=4,font=(mo_f2,14))

button3 = Button(text='記録する',font=(mo_f1,14),padx=0,pady=0,fg=c_t)
button3.bind('<ButtonRelease-1>',bookre)
button3.place_forget()

# 編集モード用
h10 = tk.Label(text='記録番号',font=(mo_f2,14))
h10.place_forget()

sel_re = ttk.Combobox(root, values = '', width=5,font=(mo_f2,14), state='readonly')
sel_re.set('')
sel_re.bind('<<ComboboxSelected>>', search_re)
sel_re.place_forget()

button4 = Button(text='上書きする',font=(mo_f1,14),padx=0,pady=0,fg=c_t)
button4.bind('<ButtonRelease-1>',bookrere)
button4.place_forget()

button5 = Button(text='削除する',font=(mo_f1,14),padx=0,pady=0,fg=c_t)
button5.bind('<ButtonRelease-1>',re_del)
button5.place_forget()
